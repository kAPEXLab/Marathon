# Question 5: Production Work Order Performance Analyzer
...
Students must implement and use the following six functions to create a modular program.
...
### 6.3 Function 3: Process All Work Orders
...
4. Call `isValidWorkOrder` for every record.
5. For valid records, call `calculatePerformanceScore` and assign the category using the documented score boundaries.
6. For invalid records, set `performanceScore` to `0` and `category` to `I`.
7. Ensure that the calculated fields of every record are updated.
8. Not read any input.
9. Not print any output.
...
### 6.4 Function 4: Find the Best Work Order
...
### 6.5 Function 5: Count Work Orders by Category
...
### 6.6 Function 6: Calculate Average Performance Score
...
### 6.7 Language-Specific Argument Passing
...
### 6.8 Responsibilities of the Main Program
...
- **Rule 10, Mandatory Function Usage:** All six mandatory functions must be implemented and used.
...
- All six mandatory functions must be implemented and used.
...
- C solutions must demonstrate passing a pointer to the work-order collection when calculated fields are updated.
- C++ solutions must demonstrate passing the work-order collection by reference when calculated fields are updated.
- Python solutions must demonstrate updating work-order objects in the collection.
